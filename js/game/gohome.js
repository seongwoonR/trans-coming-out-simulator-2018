...
function Start_Going_home(){
  ...
  Wait(1000);
	...
	function going_1(){
		...
		Wait(800);
		f("지금은 머리기르면 '역시 너도 여자였구나' 이딴 소리만 해대서.");
		y("아, 극혐.");
		Wait(1000);
		f("호르몬도, 수술도 안 한 상태에서는");
		f("하고싶은 걸 해도 역시 여자라느니 그런 소리만 한다니까.");
		f("그것 때문에 더 스트레스야.");
		Wait(800);
		y("나도 오늘 반애들 때문에 고통받았어.");
		y("내가 대학가면 머리기르고 꾸미고 다닐 것 같대.");
		Wait(800);
		...
		Choose({
			"그냥 무시했지": ignore,
			"아주 욕 한바가지를 퍼부었지.": ignore_2,
			"홧김에 커밍아웃 했어.": ignore_3
		});
}
	 ...
	 function going_2(){
		 ...
		 y("더 에너지가 소모될텐데...");
		Wait(1000);
		 f("애초에 남자랑 여자를 다르게 대해야한다는 생각이 있으니까");
		 f("트랜스젠더는 어떻게 대해야하는 지 혼란스러운 걸 거야.");
		 f("그냥 <b>'사람'</b>으로 대하면 될텐데 말이지.");
		 y("내 말이 그 말이야.");
		 Wait(1000);
		f("지금부터라도 준비 안 하면 대학때 여자로 다녀야해...");
		f("여대에 지원서 넣게 될 수도 있-");
		y("싫어어어어어어어!!")
		...
		Choose({
			"말도 안돼.": function(message){
				y(message);
				y("차라리 이해해줄만한 친구를 찾는 게 더 빠를걸.");
				f("해보기 전엔 모르는 법이라니까!");
						Waiting_End();
			},
			"그랬으면 좋겠다.": function(message){
				y(message);
				y("그럼 나도 너처럼 병원 찾아보고 있을텐데.");
				f("희망을 가져봐.");
				Waiting_End();
			},
			"음...": function(message){
				y(message);
				f("고민하는 걸 보니 커밍아웃 할 생각은 있네!");
				f("야, 힘내라.");
						Waiting_End();
			}
		});
...
function Waiting_End(){
	...
	f("그럼 성공이지. 받아들이는 건 그 사람몫이지 네 몫이 아니야.");

	Choose({
		"그런가...": accept,
		"그래도 가족이라 걱정할텐데...": concern,
		"말이야 간단하지.":anger
		});
	}
